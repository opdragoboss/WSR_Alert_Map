...
function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    { text: "Hi! I'm an AI assistant analyzing real-time NASA disaster data. Ask me anything about fires, storms, earthquakes, or disasters near you!", sender: 'bot' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemPrompt = `You are a helpful disaster information assistant. You analyze real-time NASA disaster data and help users understand risks and safety information. ${disasterContext} Guidelines: - Be concise and helpful - Use emojis appropriately (🔥 for fires, ⚡ for storms, 🌋 for volcanoes, 🌊 for floods, etc.) - If user asks about nearby disasters and location is not available, tell them to enable location - Provide safety advice when relevant - If the data shows no disasters near them, reassure them they're safe - Use the disaster data provided above to answer questions - Format responses clearly with line breaks for readability`;
  ...
  return (
    <div className="chatbot-container">
      ...
      <div className="chatbot-input">
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask me about disasters..." disabled={isLoading} />
