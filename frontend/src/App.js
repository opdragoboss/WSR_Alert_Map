...
import { fetchWildfires, fetchAirQuality, fetchAlerts } from './services/api';
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [wildfiresData, airQualityData, alertsData] = await Promise.all([
          fetchWildfires(),
          fetchAirQuality(userLocation.lat, userLocation.lng, 100000), // 100km radius
          fetchAlerts(userLocation.lat, userLocation.lng, 100000) // 100km radius
        ]);
      ...
      <div className="mobile-content">
        ...
        {activeTab === 'dashboard' && (
          <div className="dashboard-screen">
            <Dashboard data={data} lastUpdated={lastUpdated} apiStatus={apiStatus} />
          </div>
        )}
